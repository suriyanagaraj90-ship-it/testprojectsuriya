...
CKEDITOR.dialog.add("base64imageDialog", function(editor){
	...
	function imagePreviewLoad(s) {
		...
		type: "POST",
		async:false,
		url: BaseUrl+"assets/plugins/ckeditor/plugins/base64image/dialogs/upload.php",
		data: {
			base64Img: s,
			baseurl:BaseUrl,
			fileExt:FileExt
	...
	function imagePreview(src){
		...
		if(src == "base64") {
			...
			if(n && "files" in n && n.files && n.files.length > 0 && n.files[0]) {
				...
				fr.onabort = function(){ imgPreview.getElement().setHtml(""); };FileExt=n.files[0]['name'].split('.').pop();
